...
#	endif // BX_PLATFORM_
	...
	static Context sdl_ctx;

	const Event* poll()
	{
		return sdl_ctx.m_eventQueue.poll();
	}

	const Event* poll(WindowHandle _handle)
	{
		return sdl_ctx.m_eventQueue.poll(_handle);
	}

	void release(const Event* _event)
	{
		sdl_ctx.m_eventQueue.release(_event);
	}

	WindowHandle createWindow(int32_t _x, int32_t _y, uint32_t _width, uint32_t _height, uint32_t _flags, const char* _title)
	{
		bx::MutexScope scope(sdl_ctx.m_lock);
		WindowHandle handle = { sdl_ctx.m_windowAlloc.alloc() };
	...
	void destroyWindow(WindowHandle _handle)
	{
		if (UINT16_MAX != _handle.idx)
		{
			...
			bx::MutexScope scope(sdl_ctx.m_lock);
			sdl_ctx.m_windowAlloc.free(_handle.idx);
		}
	}
...
int main(int _argc, char** _argv)
{
	...
	return sdl_ctx.run(_argc, _argv);
}
