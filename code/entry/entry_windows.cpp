...
//				XInputEnable   = (PFN_XINPUT_ENABLE   )bx::dlsym(m_xinputdll, "XInputEnable"  );
	...
	static Context win_ctx;

	LRESULT CALLBACK Context::wndProc(HWND _hwnd, UINT _id, WPARAM _wparam, LPARAM _lparam)
	{
		return win_ctx.process(_hwnd, _id, _wparam, _lparam);
	}

	const Event* poll()
	{
		return win_ctx.m_eventQueue.poll();
	}

	const Event* poll(WindowHandle _handle)
	{
		return win_ctx.m_eventQueue.poll(_handle);
	}

	void release(const Event* _event)
	{
		win_ctx.m_eventQueue.release(_event);
	}

	WindowHandle createWindow(int32_t _x, int32_t _y, uint32_t _width, uint32_t _height, uint32_t _flags, const char* _title)
	{
		bx::MutexScope scope(win_ctx.m_lock);
		WindowHandle handle = { win_ctx.m_windowAlloc.alloc() };
		...
		if (UINT16_MAX != handle.idx)
		{
			...
			msg->m_title  = _title;
			msg->m_flags  = _flags;
			PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_CREATE, handle.idx, (LPARAM)msg);
		}
	...
	void destroyWindow(WindowHandle _handle)
	{
		...
		{
			PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_DESTROY, _handle.idx, 0);

			bx::MutexScope scope(win_ctx.m_lock);
			win_ctx.m_windowAlloc.free(_handle.idx);
		}
	}
	...
	void setWindowPos(WindowHandle _handle, int32_t _x, int32_t _y)
	{
		...
		msg->m_x = _x;
		msg->m_y = _y;
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_SET_POS, _handle.idx, (LPARAM)msg);
	}

	void setWindowSize(WindowHandle _handle, uint32_t _width, uint32_t _height)
	{
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_SET_SIZE, _handle.idx, (_height<<16) | (_width&0xffff) );
	}
	...
	void setWindowTitle(WindowHandle _handle, const char* _title)
	{
		Msg* msg = new Msg;
		msg->m_title = _title;
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_SET_TITLE, _handle.idx, (LPARAM)msg);
	}
	...
	void setWindowFlags(WindowHandle _handle, uint32_t _flags, bool _enabled)
	{
		...
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_SET_FLAGS, _handle.idx, (LPARAM)msg);
	}

	void toggleFullscreen(WindowHandle _handle)
	{
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_TOGGLE_FRAME, _handle.idx, 0);
	}

	void setMouseLock(WindowHandle _handle, bool _lock)
	{
		PostMessage(win_ctx.m_hwnd[0], WM_USER_WINDOW_MOUSE_LOCK, _handle.idx, _lock);
	}
	...
	int32_t MainThreadEntry::threadFunc(bx::Thread* /*_thread*/, void* _userData)
	{
		...
		PostMessage(win_ctx.m_hwnd[0], WM_QUIT, 0, 0);
		return result;
	}
...
int main(int _argc, const char* const* _argv)
{
	...
	return win_ctx.run(_argc, _argv);
}
