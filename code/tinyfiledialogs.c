...
/* #define TINYFD_NOLIB */ 
 ...
 #ifndef TINYFD_GET_WIN32_OWNER_WINDOW
  #define TINYFD_GET_WIN32_OWNER_WINDOW() GetForegroundWindow()
 #endif
#else
 #include <limits.h>
 #include <unistd.h>
...
static char * getPathWithoutFinalSlash(
        char * const aoDestination, /* make sure it is allocated, use _TINYFD_MAX_PATH */
...
static wchar_t * getPathWithoutFinalSlashW(
        wchar_t * const aoDestination, /* make sure it is allocated, use _TINYFD_MAX_PATH */
...
{ 
        wchar_t lTitleName[TINYFD_MAX_PATH];
        GetWindowTextW(hwnd, lTitleName, TINYFD_MAX_PATH);
...
static void hiddenConsoleW(wchar_t const * const aString, wchar_t const * const aDialogTitle, int const aInFront)
{
        ...
        if (aInFront)
        {
                ...
                SetWindowTextW(TINYFD_GET_WIN32_OWNER_WINDOW(), aDialogTitle);
        }
        ...
        int const aDefaultButton) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
        ...
        lBoxReturnValue = MessageBoxW(TINYFD_GET_WIN32_OWNER_WINDOW(), aMessage, aTitle, aCode);
        if (((aDialogType
        ...
        wchar_t const * const aIconType) /* L"info" L"warning" L"error" */
{
        ...
        lDialogStringLen = 3 * TINYFD_MAX_PATH_OR_CMD + lTitleLen + lMessageLen;
...
{
        static wchar_t lBuff[TINYFD_MAX_PATH_OR_CMD];
        wchar_t * lDialogString;
        FILE * lIn;
        FILE * lFile;
        int lResult;
...
#ifdef TINYFD_NOCCSUNICODE
		fgets((char *)lBuff, 2*TINYFD_MAX_PATH_OR_CMD, lIn);
#else
		fgetws(lBuff, TINYFD_MAX_PATH_OR_CMD, lIn);
#endif
		fclose(lIn);
...
{
        static wchar_t lBuff[TINYFD_MAX_PATH_OR_CMD];
        wchar_t lDirname[TINYFD_MAX_PATH_OR_CMD];
        wchar_t lDialogString[TINYFD_MAX_PATH_OR_CMD];
        wchar_t lFilterPatterns[TINYFD_MAX_PATH_OR_CMD] = L"";
        wchar_t * p;
        wchar_t * lRetval;
        int i;
        HRESULT lHResult;
        ...
        ofn.hwndOwner = TINYFD_GET_WIN32_OWNER_WINDOW();
        ofn.hInstance = 0;
        ...
        ofn.nMaxFile = TINYFD_MAX_PATH_OR_CMD;
        ofn.lpstrFileTitle = NULL;
        ofn.nMaxFileTitle = TINYFD_MAX_PATH_OR_CMD/2;
...
{
        static wchar_t lBuff[MAX_MULTIPLE_FILES*TINYFD_MAX_PATH_OR_CMD];
        ...
        else
        {
                ...
                else
                {
                        ...
                        i--;
                        p = lBuff + MAX_MULTIPLE_FILES*TINYFD_MAX_PATH_OR_CMD - 1;
                        *p = L'\0';
                        for (j = i; j >= 0; j--)
                        {
                                p -= lLengths[j];
        ...
        wchar_t const * const aDefaultPath) /* NULL or "" */
{
        ...
        bInfo.hwndOwner = TINYFD_GET_WIN32_OWNER_WINDOW();
        bInfo.pidlRoot = NULL;
        ...
        unsigned char aoResultRGB[3]) /* { 0 , 0 , 0 } */
{
        ...
        cc.hwndOwner = TINYFD_GET_WIN32_OWNER_WINDOW();
        cc.hInstance = NULL;
    ...
    int const aDefaultButton ) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
    ...
    UINT aCode ;
        ...
        lBoxReturnValue = MessageBoxA(TINYFD_GET_WIN32_OWNER_WINDOW(), aMessage, aTitle, aCode);
...
static char const * saveFileDialogWinGuiA(
    ...
    char const * const aSingleFilterDescription ) /* NULL or "image files" */
{
        char lDirname [TINYFD_MAX_PATH_OR_CMD] ;
        char lDialogString[TINYFD_MAX_PATH_OR_CMD];
        char lFilterPatterns[TINYFD_MAX_PATH_OR_CMD] = "";
        int i ;
        char * p;
        char * lRetval;
        HRESULT lHResult;
    ...
    int const aAllowMultipleSelects ) /* 0 or 1 */
{
        ...
        else 
        {
                ...
                else 
                {
                        ...
                        i--;
                        p = aoBuff + MAX_MULTIPLE_FILES*TINYFD_MAX_PATH_OR_CMD - 1 ;
                        * p = '\0';
                        for ( j = i ; j >=0 ; j-- )
                        {
                                p -= lLengths[j];
...
static int dialogPresent( )
{
        ...
        char lBuff [TINYFD_MAX_PATH_OR_CMD] ;
        FILE * lIn ;
...
{
        char lDialogString[TINYFD_MAX_PATH_OR_CMD];
        char lDialogFile[TINYFD_MAX_PATH_OR_CMD];
        FILE * lIn;
        char lBuff [TINYFD_MAX_PATH_OR_CMD] = "";
        ...
        char const * const aDefaultInput ) /* "" , if NULL it's a passwordBox */
{
        ...
        while (fgets(aoBuff, TINYFD_MAX_PATH_OR_CMD, lIn) != NULL)
        {}
        fclose(lIn);
...
{
        char lDialogString[TINYFD_MAX_PATH_OR_CMD];
        char lPathAndFile[TINYFD_MAX_PATH_OR_CMD] = "";
        FILE * lIn;
...
{
        char lDialogString [TINYFD_MAX_PATH_OR_CMD] ;
        char lString [TINYFD_MAX_PATH_OR_CMD] ;
        FILE * lIn ;
...
char const * tinyfd_inputBox(
        ...
        char const * const aDefaultInput ) /* "" , if NULL it's a passwordBox */
{
        static char lBuff [TINYFD_MAX_PATH_OR_CMD] ;
        char * lEOF;
...
#endif /* TINYFD_NOLIB */
      lEOF = fgets(lBuff, TINYFD_MAX_PATH_OR_CMD, stdin);
      if ( ! lEOF )
      {
          return NULL;
      }
...
char const * tinyfd_openFileDialog(
    ...
    int const aAllowMultipleSelects ) /* 0 or 1 */
{
        static char lBuff[MAX_MULTIPLE_FILES*TINYFD_MAX_PATH_OR_CMD];
        char const * p ;
#ifndef TINYFD_NOLIB
        if ( ( !tinyfd_forceConsole || !( GetConsoleWindow() || dialogPresent() ) )
          && ( !getenv("SSH_CLIENT") || getenv("DISPLAY") ) )
...
{
        char lBuff [TINYFD_MAX_PATH_OR_CMD] ;
        char lTestedString [TINYFD_MAX_PATH_OR_CMD] = "which " ;
        FILE * lIn ;
...
{
	static char lBuff [TINYFD_MAX_PATH_OR_CMD] ;
	char lTestedString [TINYFD_MAX_PATH_OR_CMD] ;
	FILE * lIn ;
	char * lTmp ;
        ...
        int const aDefaultButton ) /* 0 for cancel/no , 1 for ok/yes , 2 for no in yesnocancel */
{
        ...
        {
                lDialogString = (char *) malloc( TINYFD_MAX_PATH_OR_CMD + lTitleLen + lMessageLen );
        }
